...
Mechanical contract migrated from `phenix-ai#506`. `spec/nvim-integrations.md` owns product behavior. `spec/nvim-integrations-value-capability.md` and the generic `phenix-ai` value/capability SDK own runtime ABI rules.

This work starts after the authoritative client extraction is dependency-ready. It extends that client. It does not create another runtime, transport, state store, or callback model.
...
Blockwise selection remains unsupported until the canonical client implements exact rectangular snapshot semantics.
...
`ComposeReference` is the canonical compose item constructor input. Providers do not return prompt prose.
...
6. insert the selected reference through the normal compose path.
...
Renderer IDs are node-kind strings. One effective renderer exists per kind. Highest priority wins; equal priority resolves by registry ID. Failure falls back to the canonical renderer for that node only.
...
`ReviewRecord` is the exact application type. `decision` is `"accept"` or `"reject"`.

The wrapper captures `review.id` and `review.revision`. `decide` always calls the descriptor-backed runtime review-decision path with that expected revision. Adapters cannot supply another review ID or revision.

The adapter may open windows, navigate hunks, and invoke `decide`. It cannot mutate files or set terminal state locally.

If adapter presentation throws, the built-in reviewer opens and the runtime review remains pending.
...
`Callable`, `Object`, `Any`, `Never`, arbitrary `Map`, recursive/unbounded schemas, variants carrying compound payloads, and lists of compound records are unsupported in this slice. Return `unsupported_schema` with the exact schema path. Do not produce a prose fallback.
...
Every result is copied into the immutable attachment model before insertion.
...
Both methods are idempotent. `close` is called on marker deletion, sidebar teardown, buffer wipe, disconnect, or renderer replacement. Renderer failure falls back to textual attachment display.
...
Durable runtime errors remain runtime/transcript state. Notifications are presentation only.
...
The adapter never creates, resumes, closes, or mutates sessions itself. The caller invokes runtime methods after selection.
...
3. call the native client-tool registration API through `phenix_nvim.runtime` for the selected session;
...
All schemas below are ordinary client-tool schemas.
...
`false` is the default. `true` enables all three tools. All three always set `requires_permission = true`; there is no bypass switch.
...
- default tools execute on the Neovim thread through ordinary client-tool admissions;
...
After the core runtime dependency is ready, one realized Neovim check must:
...
2. Move existing built-ins behind singleton adapters without behavior change.
...
11. Reconcile PR body against exact HEAD and run exact-head validation.

## Completion gate

This slice is complete when the authoritative client dependency is green, every adapter/tool behavior is determined by this document, no checklist item requires an architectural choice, and the packaged acceptance flow passes on exact HEAD.
