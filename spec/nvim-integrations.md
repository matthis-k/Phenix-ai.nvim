...
This work makes editor-local Phenix behavior replaceable without changing runtime semantics. It extends the canonical client in this repository and the generic callable/tool admission provided by `phenix-ai`.
...
The canonical client's normalized editor context is the only source of editor coordinates and snapshots.
...
The runtime applies accepted edits through the workspace owner before reporting `Accepted`. Adapter failure leaves the review pending and opens the built-in reviewer.
...
Every source returns bytes + MIME metadata. The frontend snapshots those bytes into the canonical attachment model before insertion.
...
`phenix_nvim.tools` stores frontend-local templates. Active admissions are ordinary generic client tools.
...
This integration layer is complete when a realized Neovim flow proves:
...
- exact-head client validation.

The acceptance path uses the packaged `phenix-acp` application from `phenix-ai`. It contains no handwritten ACP or duplicate application schema.
