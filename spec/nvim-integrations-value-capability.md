...
Runtime-facing ABI rules for `spec/nvim-integrations.md`. The generic value/capability SDK and client-tool admission in `phenix-ai` are authoritative below this layer. `spec/nvim-integrations-implementation.md` fixes the frontend API and tool schemas.
...
Only the outer permission/elicitation handler registered with Phenix is lifted. That handler delegates locally to the current integration adapter and returns one typed application response.
...
The runtime-facing functions keep the exact application callable contracts:
...
A custom permission adapter returns only a frontend candidate. The outer interaction handler checks it against the application response contract before returning it across the callable boundary.

A custom elicitation renderer returns a candidate Lua value. The outer interaction handler converts it with `from_lua(request.schema, candidate)` before constructing `ElicitationResponse::Accepted`.

No adapter may return a raw `PhenixValue` tagged as trusted or bypass the final schema conversion.

## Review

`ReviewRecord` is ordinary structural application data. The revision-bound `decide` closure supplied to the adapter is frontend-local and calls the descriptor-backed `review-decide` operation.
...
Every model-visible Neovim tool uses the generic client-tool semantics provided by `phenix-ai`:
...
`phenix_nvim.tools.register` is frontend authoring sugar. It stores the definition and Lua function as a local template, then asks `phenix_nvim.runtime` to call the native generic registration API for the active session.

Third-party plugins import `phenix_nvim.tools`, not native `phenix` and not private runtime modules.

## Tool schemas

The schema passed to generic client-tool admission is exactly the schema declared in `spec/nvim-integrations-implementation.md`.
...
1. stop local polling/subscriptions through the canonical runtime teardown;
...
All Lua execution occurs through the generic bounded host callback queue in the native binding and the single Neovim polling point owned by `phenix_nvim.runtime`.
...
This frontend integration work is ABI-complete when every runtime-facing Lua function is either:

- one application interaction handler lifted through the generic callable ABI; or
- one client-tool handler lifted and admitted through the generic callable ABI.
